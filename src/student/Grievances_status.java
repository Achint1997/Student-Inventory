...
public class Grievances_status extends JFrame implements ActionListener,ItemListener{
    ...
    public String griev_status = null;

    public Grievances_status(String o) {
        griev_status = o;
        con = JdbcConnection.getConnection();
        txt3 = new JLabel("SEE GRIEVANCES STATUS");
        txt3.setBounds(30, 150, 200, 30);
        add(txt3);

        lbl = new JLabel("GRIEVANCES STATUS");
        ...
        add(cb1);
        try {
            String string = "select * from student_registration where roll_no='" + griev_status + "'";
        ...
        try {
            stmt = con.createStatement();
            res = stmt.executeQuery("select message from grievances_status where name='"+str+"'");
            while (res.next()) {
    ...
    public void actionPerformed(ActionEvent ae) {
        if ("HOME".equals(ae.getActionCommand())) {
            new After_Login_window(griev_status);
            dispose();
    }
    }
    ...
    public static void main(String []arg){
        String s=null;
        new Grievances_status("1505210002");
    }*/
}
